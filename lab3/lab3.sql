...
from departments
where budget = (
    select distinct on(budget) budget
    from departments
...
select departments.code, departments.budget from departments
...
where department = (
    select departments.code from departments where name = 'Research'
    );
